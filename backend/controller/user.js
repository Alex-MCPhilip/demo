...
router.post("/create-user", upload.single("file"), async (req, res, next) => {
  try {
    ...
    //     message: `xin chào ${user.name}, hãy nhấn vào liên kết sau để kích hoạt tài khoản của bạn: ${activationUrl}`,
    //   });
    //   res.status(201).json({
    //     success: true,
    //     message: `Vui lòng kiểm tra email: ${user.email}  của bạn để kích hoạt tài khoản! `,
    //   });
    // } catch (error) {
...
const createActivationToken = (user) => {
  return jwt.sign(user, process.env.ACTIVATION_SECRET, {
    expiresIn: "5m",// có tác dụng trong vòng 5p
  });
};
...
router.post(
  ...
  catchAsyncErrors(async (req, res, next) => {
    try {
      ...
      if (!email || !password) {
        return next(new ErrorHandler("vui lòng nhập lại email và password!", 400));
      }
